...
from snippets.jinja2 import jinjafilter


@jinjafilter
def cropped_thumbnail(
    instance, field_name, width=None, height=None, scale=None, **kwargs
):
    """Cropper"""
    ...
    try:
        return urlencode(thumbnailer.get_thumbnail(thumbnail_options).name)
...
def thumbnail(source, size, **opts):
    ...
    return urlencode(thumb.name) if thumb else ''
